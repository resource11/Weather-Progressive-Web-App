...
(function() {
  ...
  // inject weather forecast directly into JS
  ...
  app.getForecast = function(key, label) {
    ...
    url += key + '.json';
    // check if the caches object exists and request the latest data from it
    if ('caches' in window) {
    ...
    // update app.hasRequestPending flag
    ...
    request.onreadystatechange = function() {
      if (request.readyState === XMLHttpRequest.DONE) {
        if (request.status === 200) {
          ...
          // update app.hasRequestPending flag again
